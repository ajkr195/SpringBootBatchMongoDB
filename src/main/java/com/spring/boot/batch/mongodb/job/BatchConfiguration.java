...
public class BatchConfiguration {
	...
	public Job readCSVFile() {
		return jobBuilderFactory.get("readUsersCSVDataFile").incrementer(new RunIdIncrementer()).start(step1()).build();
	}

	@Bean
	public Step step1() {
		return stepBuilderFactory.get("step1").<AppUser, AppUser>chunk(10).reader(reader()).writer(writer()).build();
	}
		...
		reader.setLineMapper(new DefaultLineMapper<AppUser>() {
			{
				setLineTokenizer(new DelimitedLineTokenizer() {
					{
						setNames(new String[] { "id", "username", "password", "useremail", "userfirstname",
								"userlastname", "useraddress" });
